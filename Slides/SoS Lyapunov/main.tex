...
\begin{frame}{Rate of Convergence}
	\begin{flushleft}
		...
		\dot{x}=f(x),
		\]
		SOS can be used to find a Lyapunov function satisfying
		\[
		\beta x^\top x
		\le V(x)\le
		\alpha x^\top x,
		...
		These inequalities provide a guarantee on the convergence rate:
		\[
		\boxed{
			\|x(t)\|
			\le
		...
		Thus, the system is guaranteed to converge exponentially
		with rate
		\[
